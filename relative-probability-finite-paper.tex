...
\author{Max Sklar\\ Local Maximum Labs, Salem, NH \\ November, 2022}
\date{}
...
\begin{abstract}
This work explores and systematizes the foundations of probability theory as a relative measure rather than an absolute one. As a proof of concept, a theory of relative probability is developed on finite outcome spaces along with a deeper examination of its inner workings and generalizability. Three fundamental axioms are identified and justified which establish relative probability functions as thin categories. The various relative probability functions that can be built from these axioms are organized along with a library of examples and a system for composing them. We discuss a relative version of Bayesian inference and its digital implementation. Finally, the topological closure of relative probability space is proven to highlight its information preserving properties under limits.
\\
\\
\textit{Keywords and Phrases}: Relative Probability, Axiomatization, Foundations of Probability, Philosophy of Probability, Conditional Probability, Bayesian Inference
\\
\textit{MSC 2020 subject classifications.} Primary 60A05.
\end{abstract}

%\keywords{Relative Probability, Axiomatization, Foundations of Probability, Philosophy of Probability, Conditional Probability}
%\subjclass[2020]{60A05}
...
Let us then model probability in a non-standard way to account for the discrepancy between theory and practice. We can do so as long our new framework is logically consistent and corresponds to the advertised applications\footnote{Lyon identifies this link between application and model as the bridge principle. A new set of axioms for probability could well give rise to a new and interesting mathematics, but if that mathematics cannot be linked to any application that anyone would reasonably call probability, then it ought to go by a different name.}. This framework will take the relationships between outcomes and events as the fundamental unit instead of trying to pin down their absolute likelihoods.
...
We may also define \(\ast + m = \ast\), but there is an argument to  be made that because there are no negative numbers in this system, \(\ast + m\) should be a new pattern that only matches magnitudes greater than or equal to \(m\). Fortunately, this question can be deferred for our purposes.
...
In this case, the triangular simplex has been truncated. For higher order simplices, this appears to become exceedingly unwieldy unless some simplifying trick is developed. If it is successfully done, then a cleaner representation for members of \(\text{RPF}(K)\) becomes available.

\section{Conclusion}

Modeling probability as a \quotes{relative} measure takes on a new significance given the proliferation of computational methods. The nice properties of RPFs include formula simplification, closure under limits, and the ability to model non-standard distributions.
...
This document along with revisions is posted at github as https://github.com/maxsklar/relative-probability-finite-paper. See readme for contact information. Local Maximum Labs is an ongoing effort create an disseminate knowledge on intelligent computing. No funds, grants, or other support was received.
\end{document}
