...
\begin{enumerate}
\item We may not need to keep countable additivity as set forth in the Kolmogorov axioms; we can relax this to allow for a fair countable lottery (also known as a De Finetti Lottery\cite{de_finetti}). RPFs would provide a great way to analyze the fair countable lottery as a uniform distribution and this should be exploited!
...
Category theorists will recognize that an RPF describes a category. Specifically, and RPF describes something called a \textit{thin category} where any pair of objects have at most one morphism connecting them (per direction). The relative probability axioms can analyzed and approached through the lens of category theory in order to learn more about them.
...
\subsection{Dirichlet Distributions over \(RPF(K)\)}

The Dirichlet distribution of dimention K is a probability distribution \textbf{over} the K-dimensional simplex. This distribution's PDF may assign 0 to various edges and corners of the simplex when it's parameters are large. When it's parameters approach zero, it assigns zero probability to the interior of the simplex and infinite probability to the boundary.

The Dirichlet distribution should be re-analyzed as a (continuous) relative probability measure over the space of \(\text{RPF}(K)\). This allows the set of possible dirichlet distributions to take on its limit values.

The relative version of the dirichlet is given as follows. The there are K magnitude parameters represented by \(\mathbf{\alpha} = (\alpha_0, \alpha_1, ..., \alpha_{K-1})\) with the type of \(\mathbf{\alpha}\) being \(\mathbb{M}^K\). The relative probability compares two RPFs, \(P_1\) and \(P_2\). It makes use of a reference outcome \(r\), similar to the event comparison in definition \ref{def:event_comparison}. The simplifying powers of relative probability are on full display.

\[\mathcal{D}_{\mathbf{\alpha}}(P_1, P_2)= \prod_{k=0}^{K-1}{\left(\frac{P_1(k, r)}{P_2(k, r)}\right)}^{\alpha_k-1}\]

The Dirichlet is already extremely useful to machine learning in areas such as topic modeling\cite{blei} and prior distribution generation\cite{sklar_dirichlet}. It represents a \quotes{fuzzier} version of the categorical distribution for applications where we do not want to rely on knowing the categorical exactly at the expense of computational complexity. If the Dirichlet were allowed to include its limit values, then it would be a valuable tool for feature selection by telling us where to rely on a pure categorical parameters, where to hedge on the (fuzzier) dirichlet.
...
\bibitem{blei}Blei, D. M., Ng, A. Y., \& Jordan, M. I. (2003). Latent dirichlet allocation. the Journal of machine
