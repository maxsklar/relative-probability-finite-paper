...
\caption{An illustration of probability simplecies for K = 2, 3, and 4. These objects are respectively, a segment embedded in \(\mathbb{R}^2\), an equilateral triangle embedded in \(\mathbb{R}^3\), and a normal tetrahedron embedded in \(\mathbb{R}^4\). We make no attempt to visualize the 4D space that contains the tetrahedron.}
\end{figure}
...
The following figure gives us a roadmap of these new concepts and their relationship to each other.
...
\node (node5) [box1, below left=and -0.5 of node4,yshift=-0.38in] {Facet Probability\\ RPF\\(single impossible event)\\ Definition \ref{}};
...
\node[rectangle,rotate=45,draw=none,pattern=crosshatch, pattern color=darkgreen,align=center] at (1.2,3) (P){};

\node(Q)[rectangle,draw=none,align=center,below right=-0.3 and -0.2 of P,font=\ttfamily\footnotesize]  {Exterior Open \\ Patch (non-facet)};
...
\node(Q)[rectangle,draw=none,align=center,below right=-0.3 and 0.1 of P,font=\ttfamily\footnotesize] {Facet Patch};
...
The recent work of Censi et al.\cite{censi} concerns negative information in categories, which here corresponds to the wildcard element \(\ast\). It represents regions of the probability function that remain unassigned or uncomparable. This work could be used to subsume and further develop the idea of the indeterminate wildcard.
